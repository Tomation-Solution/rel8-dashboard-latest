...
import { membersPageDashboardData } from "../../data/dashboard/membersPageDashboardData";
...
const MembersPage = () => {
  return (
    ...
    <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 md:grid-cols-3">
      {membersPageDashboardData.map((data, index) => (
        <DashboardCard
          key={index}
          title={data.title}
          value={data.value}
    ...
    <div className="w-full">
      <TableMetaData title={"Members"}  hideFilter={true} />
      <MembersTable />
    </div>
